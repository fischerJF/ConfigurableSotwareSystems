...
public class TestPatternMatching extends JTopasTest {
  ...
  public void testNumberPattern() throws Throwable {
   ...
   // if (Configuration.BASE) {
      Reader reader = new StringReader(
          "0.95 123 -1 -123123.92382 0,32 +4,001 -123,213 -0");
      ...
      while (tokenizer.hasMoreToken()) {
        ...
        if (token.getType() != Token.EOF) {
          assertTrue(
              "Wrong token type " + Token.getTypeName(token.getType())
                  + " for: " + token.getImage(),
  ...
  public void testPatternExt() throws Throwable {
    ...
    int[] expected = { Token.NORMAL, Token.NORMAL, Token.SEPARATOR,
        Token.PATTERN, Token.SEPARATOR, Token.SEPARATOR, Token.NORMAL,
        Token.NORMAL, Token.SEPARATOR, Token.PATTERN, Token.SEPARATOR,
        Token.NORMAL, Token.NORMAL, Token.SEPARATOR, Token.PATTERN,
        Token.SEPARATOR, Token.NORMAL, Token.NORMAL, Token.SEPARATOR,
        Token.PATTERN, Token.PATTERN, Token.SEPARATOR, Token.SEPARATOR,
        Token.EOF };
    TokenizerProperties props = new StandardTokenizerProperties();
    StandardTokenizer tokenizer = new StandardTokenizer(props);
    ...
    while (tokenizer.hasMoreToken()) {
      ...
      // System.out.println(token);
      assertTrue("Wrong token type " + Token.getTypeName(token.getType())
          + " for: " + token.getImage(), token.getType() == expected[index]);
      index++;
    }
    //}
  }
  ...
  public void testPatternTokenizer() throws Throwable {
    ...
    Object[] expected = { lineComment, identifier, identifier, separator,
        identifier, separator, separator, identifier, separator, separator,
        identifier, identifier, separator, number, separator, lineComment,
        identifier, identifier, separator, number, separator, lineComment,
        identifier, identifier, separator, identifier, separator, identifier,
        separator, lineComment, identifier, identifier, separator, number,
        separator, lineComment, separator };
    ...
    while (tokenizer.hasMoreToken()) {
      ...
      if (token.getType() != Token.EOF) {
        // System.out.println(token);
        assertTrue("Wrong token companion: " + token.getCompanion() + " for: "
            + token.getImage(), token.getCompanion() == expected[index]);
      }
      index++;
    }
 //  }
  ...
  public void testFreePattern() throws Throwable {
   // if (Configuration.BASE) {
    ...
    Object[] expected = { lineComment, identifier, identifier, paraList,
        separator, identifier, identifier, separator, number, separator,
        identifier, identifier, separator, number, separator, identifier,
        identifier, separator, number, separator, lineComment, identifier,
        identifier, separator, number, number, separator, identifier, paraList,
